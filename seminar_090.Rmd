...
```{r} 
export(data_old, file = "data_for_map.xlsx") # не для русского Excel
#write.csv2(data.otd, "old.csv")
```
...
rus %>%
  ...
  addPolygons(stroke = FALSE, fillColor = ~ color_reg, # раскрашиваем области               
   ...
   addLegend(pal = pal, values = ~num, title = 'Численность') %>% # добавляем легенду
   setView(lng = 100, lat = 66, zoom = 2) # просим показывать Россию
```
...
base <- ggplot(rus_final) +
  ...
  labs(x = '', y = '', fill = 'Население 2016, млн', title = 'Население России')
base
```
...
#options(scipen = 10000) штраф за переход в научную запись в изображении чисел
